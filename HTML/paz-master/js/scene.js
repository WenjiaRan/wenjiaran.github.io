 ...
 */
  ...
  */
   class Shovel{
    ...
    static new () {
      let s = new this()
      s.img = imageFromPath(allImg.shovelback)
      s.img_shovel = imageFromPath(allImg.shovel)
      return s
    }
    // 绘制方法
    draw (cxt) {
      let self = this
      // 绘制阳光背景框
      cxt.drawImage(self.img, self.x + 250, self.y)
      cxt.drawImage(self.img_shovel, self.x + 250, self.y)
    }
  ...
  class Shovel_move{
    ...
    static new (obj) {
      let s = new this(obj)
      s.img = imageFromPath(allImg.shovel)
      return s
    }
    // 绘制方法
    draw (cxt) {
      let self = this
      cxt.drawImage(self.img, self.x + 40, self.y + 40)
    }
  }
  /**
 * 除草车类
...
class Car{
  ...
  draw (game, cxt) {
    ...
    cxt.fillText("你过来呀", self.x+10, self.y)
  }
  // 移动方法
  step (game) {
    // 只有在游戏运行状态除草车才能移动
    // game.state === game.state_RUNNING ? this.x += 1 : this.x = this.x
    ...
    for (let zombie of window._main.zombies) {
      if (zombie.row === self.row) {
        ...
        if (self.state === self.state_ATTACK) { // 当除草车启动时，清除整行僵尸
          if (zombie.x - self.x < 0&& zombie.x < 950) {
            zombie.life = 0
...
class Plant extends Role{
    constructor (obj) {
      ...
      let p = {
        ...
        sunTimer_spacing: 15,                                // 生成阳光时间间隔（秒）
      }
    ...
    setSunTimer () {
      ...
      self.sunTimer = setInterval(function () {
        ...
        sun.animate(keyframes1,keyframesOptions)
         this.sunxiaoshi = setTimeout(()=>{
                let keyframes3 = [                                        // 阳光移动动画 keyframes
                    { transform: 'translate(0,0)', opacity: 0 },
                    { offset: .3,transform: 'translate(0,0)', opacity: 1 },
                    { offset: 1,transform: 'translate(0,0)', opacity: 0 },
                ]
                if(sun.parentNode !== null){
                    sun.parentNode.removeChild(sun)
                }else{
                    return false
                }
                sun.animate(keyframes3,keyframesOptions)
            },4000)
            ...
            let audio = document.querySelector(".audio_plant")
            audio.src = "./images/music/points.mp3"
            audio.autoplay = true
            clearTimeout(this.sunxiaoshi)
            sun.animate(keyframes2,keyframesOptions)
            // 动画完成，清除阳光元素
            sun.parentNode.removeChild(sun)
            // 增加阳光数量
    ...
    init () {
      ...
      setPlantFn = {
        sunflower () {  // 向日葵
          self.idle = Animation.new(self, 'idle', 6)
          // 定时生成阳光
          self.canSetTimer && self.setSunTimer()
        },
        peashooter () { // 豌豆射手
          self.canShoot = true
          self.idle = Animation.new(self, 'idle', 11)
        ...
        wallnut () { // 坚果墙
          ...
          self.life = 12
          // 创建坚果三种不同血量下的动画对象
          self.idleH = Animation.new(self, 'idleH', 9)
...
class Zombie extends Role{
    ...
    init () {
      ...
      // 移动
      self.run = Animation.new(self, 'run', 6)
      // 攻击
      self.attack = Animation.new(self, 'attack', 8)
      // 炸死
